...
import { getWeeklyIntelligencePackages } from '@/lib/editorial-sprint';

export function WeeklyIntelligence() {
  const { t, language } = useLanguage();
  const shouldReduceMotion = useReducedMotion();
  const localizedCards = getWeeklyIntelligencePackages((language as 'en' | 'es' | 'fr' | 'de') ?? 'en');
  const cardStyles = [
    {
      icon: ShieldCheck,
      color: 'text-brand',
        ...
        >
          {localizedCards.map((item, i) => {
            ...
            <motion.div 
              ...
              className="group relative rounded-[2rem] bg-bg-card border border-white/10 hover:border-brand/40 transition-all duration-500 overflow-hidden"
              whileHover={shouldReduceMotion ? undefined : { y: -3 }}
              style={{ willChange: shouldReduceMotion ? undefined : 'transform' }}
            >
              <Link href={item.href} className="block p-8 h-full">
                ...
                <div className="relative z-10">
                  <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center mb-8 transition-transform duration-500 group-hover:scale-110", style.bg)}>
