...
export function TrustStrip() {
  ...
  return (
    ...
    <motion.div
      ...
      className="w-full h-12 md:h-14 bg-bg-surface overflow-hidden flex items-center border-y border-white/5 relative z-0 select-none"
    >
      {/* Desktop View */}
      <div className="hidden lg:flex items-center justify-center w-full max-w-7xl mx-auto gap-4 xl:gap-6 px-4">
        {phrases.map((phrase, i) => (
          <React.Fragment key={i}>
            <span className="text-[10px] xl:text-[11px] font-semibold uppercase tracking-wider text-brand/80 whitespace-nowrap">
              {phrase}
            </span>
            {i < phrases.length - 1 && (
