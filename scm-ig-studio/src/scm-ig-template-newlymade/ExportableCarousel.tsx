...
export function ExportableCarousel({ 
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const isPortrait = firstSlide && firstSlide.clientHeight > 1100
      const exportHeight = isPortrait ? 1350 : 1080
      ...
      for (let i = 0; i < slides.length; i++) {
        ...
        // Temporarily remove transform so html2canvas renders the true resolution
        ...
        if (i > 0) {
          pdf.addPage([1080, exportHeight], 'portrait')
        }
        
        pdf.addImage(imgData, 'JPEG', 0, 0, 1080, exportHeight)
      }
